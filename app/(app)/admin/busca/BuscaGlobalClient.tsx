...
export function BuscaGlobalClient() {
  ...
  const run = () => {
    ...
    startTransition(async () => {
      try { setRows(await buscaGlobal(q)); }
          ...
          <tbody>
            {rows.map((r) => (
              <tr key={r.id} className="border-t border-slate-50">
                <td className="py-2"><span className="font-medium text-perlog-navy">{r.nome}</span></td>
                <td className="text-perlog-slate">{r.cpf}</td>
                <td><Badge variant={statusVariant(r.status)}>{r.status}</Badge></td>
                <td className="text-perlog-slate">{new Date(r.dataHora).toLocaleString('pt-BR')}</td>
              </tr>
            ))}
          </tbody>
        </table>
