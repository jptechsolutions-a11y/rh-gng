...
import { importarCursos, type ImportarCursosResult } from '@/actions/indicadores/cursos';

export function ImportarCursosDialog() {
  const [open, setOpen] = useState(false);
  const [arquivo, setArquivo] = useState<File | null>(null);
  const [resultado, setResultado] = useState<ImportarCursosResult | null>(null);
  ...
  function submit() {
    ...
    start(async () => {
      try {
        const r = await importarCursos(fd);
        setResultado(r);
        ...
        >
          ...
          <div className="p-6 pt-7">
            ...
            <Dialog.Description className="text-[13px] text-conecta-muted mt-1">
              Selecione o arquivo <span className="font-semibold">TREINAMENTOS OBRIGATÓRIOS</span> (.xls ou .xlsx).
              A base atual será copiada para a base anterior antes de ser substituída pelos novos dados.
            </Dialog.Description>
