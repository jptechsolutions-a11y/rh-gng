...
import type { DadosFeriados } from '@/actions/indicadores/feriados';
import type { ResumoFilialFeriados } from '@/lib/indicadores/feriados-queries';
import { CardsResumoFeriados } from './CardsResumo';
import { RoscaTop5Feriados } from './RoscaTop5';
...
export function FeriadosView({ dados, podeImportar, multiFilial }: { dados: DadosFeriados; podeImportar: boolean; multiFilial: boolean }) {
  ...
  const resumoEfetivo = useMemo(() => {
    ...
    const valorTotal = detalhadoFiltrado.reduce((a, r) => a + r.valorTotal, 0);
  ...
  const porFilialEfetivo = useMemo<ResumoFilialFeriados[]>(() => {
    ...
    for (const r of detalhadoFiltrado) {
      ...
      const cur = map.get(key) ?? {
        ...
        qtd: 0, chapas: new Set<string>(), valor: 0,
      };
      cur.qtd += r.qtdPendencias;
    ...
    return [...map.entries()]
      .map(([k, v]) => ({
        ...
        valorTotal: Math.round(v.valor * 100) / 100,
      }))
  ...
  return (
    <div className="space-y-5">
      ...
      <CardsResumoFeriados r={resumoEfetivo} />
      ...
      {subTab === 'indicadores' ? (
        <>
          ...
          <TabelaResumoFilialFeriados rows={porFilialEfetivo} />
        </>
      ) : (
        <TabelaDetalhadoFeriados
