...
import { importarFeriados, type ImportarFeriadosResult } from '@/actions/indicadores/feriados';

export function ImportarFeriadosDialog() {
  const [open, setOpen] = useState(false);
  const [arquivo, setArquivo] = useState<File | null>(null);
  const [resultado, setResultado] = useState<ImportarFeriadosResult | null>(null);
  ...
  function submit() {
    ...
    start(async () => {
      try {
        const r = await importarFeriados(fd);
        setResultado(r);
  ...
  return (
    ...
    >
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content
          className="fixed left-1/2 top-1/2 z-50 -translate-x-1/2 -translate-y-1/2 w-[min(640px,95vw)] overflow-hidden rounded-xl bg-white border border-conecta-primary/10"
        ...
        >
          ...
          <div className="p-6 pt-7">
            ...
            <Dialog.Description className="text-[13px] text-conecta-muted mt-1">
              Selecione o arquivo <span className="font-semibold">Folgas Feriados Pendentes</span> (.xls ou .xlsx).
              Todas as abas serão lidas e unificadas, mantendo apenas as linhas com <span className="font-semibold">BANDEIRA = PERLOG</span>.
            ...
            {resultado && (
              ...
              >
                <div className="font-display text-sm font-semibold text-conecta-primary">
                  ...
                  <span className="text-conecta-muted font-normal ml-2 text-[12px]">
                    (de {resultado.totalLidoBruto.toLocaleString('pt-BR')} brutas · {resultado.totalNaoPerlog.toLocaleString('pt-BR')} ignoradas por bandeira ≠ PERLOG)
                  </span>
                </div>
                <div className="mt-1 text-[12px] text-conecta-muted">
                  Abas processadas: <span className="font-mono">{resultado.abasProcessadas.join(', ')}</span>
                </div>
